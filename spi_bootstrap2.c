...
static void addAttributeToGroup(MyGroup *group, float4 quantity, float4 orderkey, float4 extendedprice);
...
Datum spi_bootstrap_array_all(PG_FUNCTION_ARGS) {
    ...
    for (i = 0; i < SPI_processed; i++) {
        ...
        if (groupIndex != -1) { 
            addAttributeToGroup(&groupsContext.groups[groupIndex],quantity,orderkey,extendedprice);
        }
